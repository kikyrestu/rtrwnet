...
walkDir('D:/PROJECT/RT-RWNET_PAKAAM/frontend/src/app', (filePath) => {
    if (filePath.endsWith('.tsx') || filePath.endsWith('.ts')) {
        ...
        if (content.includes("import Swal from 'sweetalert2';")) {
            content = content.replace("import Swal from 'sweetalert2';", "import Swal from '@/lib/swal';");
            edited = true;
        }
        if (content.includes("background: '#1e293b'")) {
            content = content.replace(/background: '#1e293b',?/g, "");
            edited = true;
        }
        if (content.includes("color: '#f8fafc'")) {
            content = content.replace(/color: '#f8fafc',?/g, "");
            edited = true;
        }
        if (edited) {
